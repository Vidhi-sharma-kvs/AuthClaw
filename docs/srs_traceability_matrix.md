...
| RBAC | Multi-tenant role-based access control | Implemented for code scope | Centralized backend permission matrix covers all FastAPI routes, production RBAC enforcement gate exists, frontend protected routes remain, tenant admin readiness endpoint exposes matrix, and automated tests prove role coverage | Production deployments must enable `AUTHCLAW_ENABLE_RBAC_ENFORCEMENT=true` or run with `AUTHCLAW_ENV=production`; customer-specific custom roles remain configuration | Medium | Auth service, permission model, tests | Complete for code-only scope |
...
| Public Trust Center | Public compliance/trust page | Implemented for code scope | Public frontend Trust Center, cached signed runtime state route, health endpoint, metrics/audit/provider/certificate status, verification metadata, and tests exist | Public production claims still require live deployment configuration and customer certificate/provider evidence | Medium | Evidence engine, signed exports, public verifier | Complete for code-only scope |
...
| NFR-2.1 | Tenant isolation through RLS or physical isolation | Implemented for logical isolation | Tenant isolation coverage report verifies every tenant table has tenant_id/RLS/forced RLS/policies or explicit system-table exemption; tests assert zero unclassified gaps | Physical tenant isolation remains intentionally out of scope per current instruction | Medium | Schema review, migrations, tests | Complete for logical isolation |
| NFR-2.2 | Envelope encryption AES-256-GCM via KMS or Vault for client credentials | Implemented for code scope | Secret manager supports local AES-GCM, AWS Secrets Manager/KMS envelope paths, Vault transit envelope paths, rotation operations, startup validation, health/readiness endpoint, and customer-managed KMS Terraform | Real AWS KMS/Vault connectivity and rotation drills require deployed enterprise infrastructure | Medium | KMS/Vault, Terraform, credential APIs | Complete for code-only scope |
...
| Terraform | ECS, ALB, CloudWatch, RDS, S3, secrets | Implemented for code scope | Terraform baseline includes ECS, ALB, CloudWatch, RDS, S3, Secrets Manager, customer-managed KMS, Redis, MSK/analytics controls, Route53 DR, backup copy, S3 replication, and OPA sidecar definition | `terraform apply` and cloud-specific tuning require live AWS accounts | Medium | Terraform modules, AWS services | Complete for code-only scope |
...
| Disaster recovery | Backup, restore, failover, runbooks | Implemented for code scope | DR Terraform controls, validation harness, runbook, chaos scenarios, production-readiness report, configuration validation, and RTO/RPO documentation exist | Regional failover evidence must be produced from a deployed two-region environment | Large | Terraform, data stores, ops runbooks | Complete for code-only scope |
| Testing coverage | Unit, integration, gateway, policy, auth, document tests | Implemented for code scope | Backend readiness tests, RBAC matrix tests, tenant isolation tests, Trust Center runtime tests, Go/provider/policy/document/performance tests, and real-app Playwright E2E harness exist | Playwright execution requires local/staging app credentials and installed Playwright runtime | Large | Test harnesses, implemented features | Complete for code-only scope |
