...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-950/90 border-r border-white/5 backdrop-blur-xl transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:inset-0 flex flex-col`}>

      {/* Logo Section */}
      <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 bg-slate-950/40 shrink-0">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-gradient-to-tr from-violet-600 to-fuchsia-600 rounded-lg shadow-lg shadow-violet-500/20">
            <ShieldAlert className="w-5 h-5 text-white" />
          </div>
          <span className="font-bold text-lg bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent">
            AuthClaw
          </span>
        </div>
      ...
      {/* Scrollable Nav Area — nav items + user card */}
      <div className="flex-1 overflow-y-auto">
          ...
          {allowedNavItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.path}
                className={({ isActive }) =>
                  `flex items-center gap-3.5 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group ${
                    isActive
                      ? 'bg-gradient-to-r from-violet-600/20 to-fuchsia-600/10 border-l-2 border-violet-500 text-white shadow-inner shadow-white/5'
        ...
        {/* User Card — directly below Developer Portal, inside the scroll area */}
        {user && (
          <div className="px-4 pt-3 pb-2">
            <div className="flex items-center gap-3 bg-white/2 border border-white/5 p-2.5 rounded-lg">
              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-violet-600 to-fuchsia-600 flex items-center justify-center font-bold text-white text-sm uppercase shadow-inner shrink-0">
      ...
      <div className="p-4 border-t border-white/5 bg-slate-950/40 text-center shrink-0">
        <span className="text-[10px] text-gray-500 tracking-wider">AuthClaw v2.0.0 • Production MVP</span>
      </div>
